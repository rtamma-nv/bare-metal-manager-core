...
async fn update_instance_extension_services_config(
    ...
    // per-(tenant, service VPC) index and endpoint /127s. Runs on the merged
    // config (assignment skips terminating entries), so complete-replace
    // updates reconstruct bindings from the reservation rows. An empty DPU
    // list is rejected inside assign_service_vpc_bindings, and only when a
    // new binding actually needs it — plain (non-VPC) service changes must
    // not fail on a transient snapshot-load gap.
    //
    ...
    let mut new_extension_services_config = new_extension_services_config;
    ...
    crate::instance::assign_service_vpc_bindings(
        &mut new_extension_services_config,
