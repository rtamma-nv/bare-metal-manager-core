...
pub(crate) async fn delete(
    ...
    match &current_service.service_type {
        ...
        ExtensionServiceType::KubernetesPod => {
            let (credential_versions, removed_attachment_ids) =
                delete_kubernetes_pod(&mut txn, service_id, current_service, &versions).await?;

            txn.commit().await?;

            // Best-effort removal of any DPF service-VPC CRs left by the
            // deleted reservations. The rows are already gone; the ownership
            // labels let a later sweep collect anything missed here.
...
///
/// Returns the versions whose Vault credentials need cleanup and the
/// attachment ids whose DPF service-VPC CRs need sweeping — both are the
/// caller's post-commit, best-effort work.
async fn delete_kubernetes_pod(
    txn: &mut db::Transaction<'_>,
    service_id: ExtensionServiceId,
    service: &ExtensionService,
    versions: &[ConfigVersion],
) -> Result<(Vec<ConfigVersion>, Vec<AttachmentId>), Status> {
    if service.deleted.is_some() {
        return Err(CarbideError::NotFoundError {
            kind: "extension_service",
            id: service_id.to_string(),
    ...
    // best-effort by the caller after the transaction commits.
    ...
    if !deleted_versions.is_empty() {
            ...
            .is_empty()
        {
            ...
            if let Some(service_vpc_id) = service.service_vpc_id {
    ...
    Ok((credential_versions, removed_attachment_ids))
}
