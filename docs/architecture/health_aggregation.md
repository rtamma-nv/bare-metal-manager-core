...
- Static power-shelf endpoints can include serial number and NVLink domain UUID metadata
...
- `[sinks.prometheus]` adds _machine_ metadata as metric labels named `machine_id`, `system_uuid`, `serial_number`, `rack_id`, `machine_slot_number`, `machine_tray_index`, and `nvlink_domain_uuid`. _Switch_ metadata labels are `switch_id`, `serial_number`, `rack_id`, `switch_slot_number`, `switch_tray_index`, and `nvlink_domain_uuid`. _Power-shelf_ metadata labels are `power_shelf_id`, `serial_number`, `rack_id`, and `nvlink_domain_uuid`; each is omitted when the corresponding metadata is unavailable. Static endpoint custom labels keep their configured names.
...
- `[sinks.otlp]` names resource attributes in the same `lower_snake_case` style as the Prometheus labels, but the two sinks do not share every name. Prometheus emits one `serial_number` label for machine, switch, and power-shelf metadata, and identifies the endpoint with `endpoint_key`, `endpoint_ip`, and `endpoint_mac`; OTLP uses the type-specific names listed here instead. It adds the string resource attributes `collector_type` and either `bmc_endpoint` and `bmc_ip`, or `switch_endpoint` and `switch_ip` for host-side switch collection. Typed inventory adds the strings `component_type` and, when present, `rack_id`. _Machine_ metadata attributes are the strings `machine_id`, `system_uuid`, `machine_serial`, `driver_version`, and `nvlink_domain_uuid`, plus the integers `machine_slot_number` and `machine_tray_index`. _Switch_ metadata attributes are the strings `switch_id`, `switch_serial_number`, `switch_endpoint_role`, and `nvlink_domain_uuid`, the boolean `switch_is_primary`, and the integers `switch_slot_number` and `switch_tray_index`. _Power-shelf_ metadata attributes are the strings `power_shelf_id`, `power_shelf_serial_number`, and `nvlink_domain_uuid`; each is omitted when the corresponding metadata is unavailable. Static endpoint custom labels are string resource attributes and keep their configured names.
...
| `powersupply_id` | string | Optional | For `Sensor` probes, the Redfish `PowerSupply` ID the sensor belongs to. Omitted for sensors outside a power supply and for other probes. |
| `physical_context` | string | Optional | For `Sensor` probes, the sensor's Redfish `PhysicalContext` in snake case, for example `power_supply`. Omitted for other probes. |

When `health_report.alerts` is present, every JSON object carries the same
`probe_id`, optional `target`, optional `powersupply_id`, and optional
`physical_context` fields, plus at least these fields:
