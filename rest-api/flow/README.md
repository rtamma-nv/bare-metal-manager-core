...
Flow integrates with the Powershelf Manager (PSM) service to manage power shelves. PSM runs as a sidecar container in the same pod as Flow.
...
Flow connects to PSM via the `PSM_API_URL` environment variable, which is set to `localhost:50052` by default (PSM sidecar port).
